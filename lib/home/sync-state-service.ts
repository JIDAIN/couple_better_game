...
export function guardRemoteReload({
  ...
  if (hasUnsyncedChanges && !allowDirtyOverwrite) {
    return {
      ok: false,
      reason: "当前有未同步修改，请先同步，或再次确认用 GitHub 数据覆盖本地",
  ...
  if (hasLocalData && !hasLastSyncedAt && !allowDirtyOverwrite) {
    return {
      ok: false,
      reason: "本地已有数据但缺少上次同步记录，为避免覆盖，请先同步本地数据",
...
export function shouldRetrySync({
  ...
  if (errorCode === "WRONG_PASSWORD" || errorCode === "MISSING_PASSWORD") {
    return false;
  }
...
export function shouldRunPendingAutoSync({
  ...
  return errorCode !== "WRONG_PASSWORD" && errorCode !== "MISSING_PASSWORD";
}
