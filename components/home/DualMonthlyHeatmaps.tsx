...
import { AppButton, AppCard, AppInput, AppRoleAvatar } from "../ui";
...
function formatStartDateLabel(value: string) {
...
export function DualMonthlyHeatmaps() {
  ...
  return (
    <AppCard
      variant="hero"
      className="relative w-full overflow-hidden sm:p-5"
      aria-label="当月成就"
    >
      <div className="relative mx-auto max-w-[28rem]">
        <AppCard variant="panel" className="text-center sm:px-5">
          <div className="flex items-center justify-between gap-2">
            <AppButton
              type="button"
              onClick={handlePrevMonth}
              className="is-ghost inline-flex h-8 w-8 items-center justify-center text-sm font-bold"
              aria-label="查看上个月"
            >
              &lt;
            </AppButton>
          ...
          <div className="heatmap-meta-line">
            <span>本周已坚持 {weeklySuccessDays} 天 ☀️</span>
            <details className="heatmap-start-date-edit">
              <summary>开始日 {formatStartDateLabel(visibleStartDate)}</summary>
              <AppInput
                type="date"
                ...
                inputSize="small"
                className="heatmap-start-date-input"
              />
            </details>
          </div>
        </AppCard>
      ...
      <div className="relative mt-4 grid grid-cols-1 gap-3 lg:grid-cols-[repeat(auto-fit,minmax(18rem,1fr))]">
        <PlayerHeatmap
          title={<><AppRoleAvatar role="fish" size={16} /> 的成长热力图</>}
          playerShort={<AppRoleAvatar role="fish" size={18} />}
          playerLabel="鱼鱼"
          grid={fishGrid}
        />
        <PlayerHeatmap
          title={<><AppRoleAvatar role="cat" size={16} /> 的成长热力图</>}
          playerShort={<AppRoleAvatar role="cat" size={18} />}
          playerLabel="猫猫"
          grid={catGrid}
        />
      </div>

      <AppCard variant="soft" className="relative mt-3.5">
        <HeatmapLegend />
      </AppCard>
    </AppCard>
  );
