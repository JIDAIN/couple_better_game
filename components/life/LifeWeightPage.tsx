...
function sortWeightRecords(records: WeightRecord[]) { return [...records].sort((a, b) => recordTimestamp(b) - recordTimestamp(a)); }
...
function periodBounds(year: number, period: Period) {
  ...
  if (period === "month") {
    ...
    return { start: `${year}-${String(month).padStart(2, "0")}-01`, end: `${year}-${String(month).padStart(2, "0")}-${String(daysInMonth(year, month)).padStart(2, "0")}` };
  }
  if (period === "quarter") {
    const startMonth = Math.floor(reference.getMonth() / 3) * 3 + 1;
    const endMonth = startMonth + 2;
    return { start: `${year}-${String(startMonth).padStart(2, "0")}-01`, end: `${year}-${String(endMonth).padStart(2, "0")}-${String(daysInMonth(year, endMonth)).padStart(2, "0")}` };
  }
  const end = reference;
...
function WeightChart({ points }: { points: DailyWeightPoint[] }) {
  if (points.length < 2) {
    return <div className="grid h-40 place-items-center rounded-[var(--life-radius-control)] bg-[var(--life-surface-soft)] px-4 text-center text-xs font-bold text-[var(--life-text-muted)]">再多记录几次，就能慢慢看见趋势。</div>;
  }
  const weights = points.map((item) => item.weightKg);
  const min = Math.min(...weights);
  const max = Math.max(...weights);
  const spread = Math.max(max - min, 1);
  const coords = points.map((item, index) => ({ x: (index / (points.length - 1)) * 100, y: 88 - ((item.weightKg - min) / spread) * 68, item }));
  ...
  return (
    <div className="life-weight-chart rounded-[var(--life-radius-control)] bg-[var(--life-surface-soft)] p-3">
      ...
      <div className="mt-1 flex justify-between text-[10px] font-bold text-[var(--life-text-muted)]"><span>{shortDate(points[0].date)}</span><span>{min.toFixed(1)}–{max.toFixed(1)} kg</span><span>{shortDate(points[points.length - 1].date)}</span></div>
    </div>
  );
}
...
export function LifeWeightPage() {
  ...
  function switchRole(next: AppRoleSwitchValue) {
    if (next === role) return;
    setEditing(null); setWeight(""); setTargetEditing(false); setRole(next);
  }
  function edit(record: WeightRecord) {
    if (!canEdit || record.linkedDailyRecordSideId) return;
    setEditing(record); setDate(record.measurementDate); setTime(record.measuredAt ? new Date(record.measuredAt).toTimeString().slice(0, 5) : ""); setWeight(String(record.weightKg));
  }
  function resetForm() { setEditing(null); setDate(localDate()); setTime(localTime()); setWeight(""); }

  async function save() {
    if (!partnerKey || !canEdit) return;
    const value = Number(weight);
    if (!Number.isFinite(value) || value <= 0 || value >= 500) { setError("请输入有效体重（0–500 kg）"); return; }
    const measuredAt = measuredAtFrom(date, time);
    if (!measuredAt) { setError("请填写精确记录时间"); return; }
    const payload: WeightWritePayload = { partnerKey, measurementDate: date, measuredAt, weightKg: value, note: null };
    ...
    try {
      ...
      setYear(Number(saved.measurementDate.slice(0, 4))); resetForm();
    } catch (cause) { setError(cause instanceof WeightApiError ? cause.message : "体重记录暂时没有保存成功"); }
  ...
  async function saveTarget() {
    ...
    setSaving(true); setError(null);
    try { const saved = await patchLifeSettings({ targetWeightKg: value }); settingsQuery.update(saved); setTargetEditing(false); }
  ...
  async function remove(record: WeightRecord) {
    if (!canEdit || record.linkedDailyRecordSideId) return;
    setSaving(true); setError(null);
    try { await deleteWeightRecord(record.id); recordsQuery.update((current) => (current ?? []).filter((item) => item.id !== record.id)); if (editing?.id === record.id) resetForm(); }
    catch (cause) { setError(cause instanceof WeightApiError ? cause.message : "这条记录暂时没有删除成功"); }
    finally { setSaving(false); }
  }

  if (!partnerKey) return <AppPageShell title="体重" subtitle="正在确认当前账号…"><section className="life-surface life-section-card text-sm text-[var(--life-text-muted)]">正在确认当前账号…</section></AppPageShell>;

  return (
    <AppPageShell title="体重" subtitle="慢慢看见自己的变化。" actions={<Link href="/nest" className="life-back-link">返回小窝</Link>}>
      <div className="grid gap-3">
        <AppRoleSwitch value={role} onChange={switchRole} />

        <section className="life-surface life-section-card life-weight-hero-v2">
          <div className="life-weight-current"><p className="text-[10px] font-extrabold text-[var(--life-text-muted)]">当前体重</p><p className="mt-1 text-3xl font-black tabular-nums text-[var(--life-text)]">{latest ? Number(latest.weightKg).toFixed(1) : "--"}<span className="ml-1 text-sm font-bold text-[var(--life-text-body)]">kg</span></p><p className="mt-1 text-[11px] font-bold text-[var(--life-text-muted)]">{latestChange == null ? "暂无上次变化" : `较上次 ${latestChange > 0 ? "+" : ""}${latestChange.toFixed(1)} kg`}</p></div>
          <div className="life-weight-target"><p className="text-[10px] font-extrabold text-[var(--life-text-muted)]">目标体重</p><p className="mt-1 text-xl font-black tabular-nums text-[var(--life-text)]">{targetWeight == null ? "--" : Number(targetWeight).toFixed(1)}<span className="ml-1 text-xs font-bold text-[var(--life-text-body)]">kg</span></p>{canEdit ? <button type="button" className="mt-1 text-[10px] font-extrabold text-[var(--life-teal-strong)]" onClick={() => { setTargetDraft(targetWeight == null ? "" : String(targetWeight)); setTargetEditing(true); }}>编辑目标</button> : <span className="mt-1 block text-[10px] text-[var(--life-text-muted)]">Ta 自己设置</span>}</div>
        </section>
        ...
        <section className="life-surface life-section-card life-data-section">
          <div className="life-weight-year-nav mb-3"><button type="button" className="life-year-arrow" aria-label="上一年" onClick={() => setYear((value) => value - 1)}><span aria-hidden>‹</span></button><strong>{year} 年</strong><button type="button" className="life-year-arrow" aria-label="下一年" onClick={() => setYear((value) => value + 1)}><span aria-hidden>›</span></button></div>
          <div className="life-weight-period-switch mb-3 grid grid-cols-4 gap-1">{PERIODS.map((item) => <button key={item.key} type="button" onClick={() => setPeriod(item.key)} className={period === item.key ? "is-active" : ""}>{item.label}</button>)}</div>
          <WeightChart points={dailyPoints} />
        </section>

        {canEdit ? <section className="life-surface life-section-card life-data-section">
          <div className="mb-3 flex items-center justify-between gap-3"><p className="text-sm font-extrabold text-[var(--life-text)]">{editing ? "编辑记录" : "记录体重"}</p>{editing ? <button type="button" onClick={resetForm} className="text-xs font-extrabold text-[var(--life-teal-strong)]">取消编辑</button> : null}</div>
          <div className="grid grid-cols-3 gap-2"><label className="col-span-3 grid gap-1 text-xs font-bold text-[var(--life-text-body)] sm:col-span-1">日期<AppInput type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label className="grid gap-1 text-xs font-bold text-[var(--life-text-body)]">时间<AppInput type="time" value={time} onChange={(event) => setTime(event.target.value)} /></label><label className="grid gap-1 text-xs font-bold text-[var(--life-text-body)]">体重 kg<AppInput inputMode="decimal" placeholder="52.6" value={weight} onChange={(event) => setWeight(event.target.value)} /></label></div>
        ...
        <section className="life-surface life-section-card life-data-section">
          <div className="mb-3 flex items-center justify-between gap-3"><p className="text-sm font-extrabold text-[var(--life-text)]">{year} 年记录</p><span className="text-[10px] font-bold text-[var(--life-text-muted)]">{yearRecords.length} 条</span></div>
          <div className="life-weight-history-list">
            ...
            {yearRecords.map((record) => <div key={record.id} className="life-weight-history-row"><div className="life-weight-history-date"><strong>{Number(record.measurementDate.slice(5, 7))}月{Number(record.measurementDate.slice(8, 10))}日</strong><span>{exactTime(record)}</span></div><div className="life-weight-history-value"><strong>{Number(record.weightKg).toFixed(1)}</strong><span>kg</span></div>{canEdit && !record.linkedDailyRecordSideId ? <div className="life-weight-history-actions"><button type="button" onClick={() => edit(record)}>编辑</button><button type="button" disabled={saving} onClick={() => void remove(record)}>删除</button></div> : <span className="text-[9px] font-bold text-[var(--life-text-muted)]">{record.linkedDailyRecordSideId ? "旧数据" : "只读"}</span>}</div>)}
          </div>
        </section>
