...
export function LifeMealEditorPage() {
  ...
  useEffect(() => {
    ...
    fetchMeals({ mealDate: initialDate, partnerKey }).then((records) => {
      ...
      setMeal(found); setDate(found.mealDate); setMealType(found.mealType); setSnackPeriod(found.mealType === "snack" ? found.snackPeriod : null); setTime(found.eatenAt ? new Date(found.eatenAt).toTimeString().slice(0, 5) : ""); setNote(found.note ?? ""); setItems(found.items.length ? found.items.map(fromItem) : [emptyItem()]);
  ...
  function choosePhoto(file: File | null) {
    ...
    setPhotoFile(file); setRemovePhoto(false); if (photoPreviewUrl) URL.revokeObjectURL(photoPreviewUrl); setPhotoPreviewUrl(URL.createObjectURL(file));
  }
  function clearPhoto() { setPhotoFile(null); if (photoPreviewUrl) URL.revokeObjectURL(photoPreviewUrl); setPhotoPreviewUrl(null); setRemovePhoto(Boolean(meal?.photoPath)); }
      ...
      <div className="grid gap-3">
        ...
        <section className="life-surface life-section-card life-meal-editor-overview">
          ...
          <div className="grid grid-cols-[minmax(0,0.95fr)_minmax(0,1.25fr)] gap-3">
            <label className="life-meal-editor-photo relative aspect-[4/3] cursor-pointer overflow-hidden rounded-[var(--life-radius-control)] bg-[var(--life-surface-warm)]"><Image unoptimized src={photoSrc} alt={customPhotoVisible ? "当前餐食照片" : "默认餐食卡通图"} fill sizes="(max-width: 480px) 42vw, 190px" className="object-cover" /><span className="life-meal-photo-action">{customPhotoVisible ? "更换照片" : "＋ 上传照片"}</span><input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" className="sr-only" disabled={saving} onChange={(event) => choosePhoto(event.target.files?.[0] ?? null)} /></label>
