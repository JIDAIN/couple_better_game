...
export function LifeMealEditorPage() {
  ...
  const preview = useMemo(() => items.reduce((sum, item) => {
  ...
  async function save() {
    ...
    for (const item of items) {
      ...
      if (calories == null || !Number.isInteger(calories)) {
        setError("当前版本每项食物还需要填写整数热量；下一步会放宽为可选");
        return;
      }
      parsedItems.push({
        foodId: null,
    ...
    const payload: MealWritePayload = {
      ...
      totalCaloriesKcal: parsedItems.reduce((sum, item) => sum + item.caloriesKcal, 0),
      calorieMinKcal: null,
      calorieMaxKcal: null,
      note: note.trim() || null,
  ...
  return (
    <AppPageShell title={meal ? "编辑一餐" : "添加一餐"} subtitle="记录实际吃了什么；营养数据可以逐项补充。">
      ...
      <div className="grid gap-3">
          ...
          <section key={item.key} className="life-surface life-section-card">
            ...
            <div className="grid gap-2.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <AppInput inputMode="numeric" placeholder="热量 kcal" value={item.caloriesKcal} onChange={(event) => updateItem(item.key, { caloriesKcal: event.target.value })} />
                <AppInput inputMode="decimal" placeholder="碳水 g" value={item.carbsG} onChange={(event) => updateItem(item.key, { carbsG: event.target.value })} />
                <AppInput inputMode="decimal" placeholder="蛋白质 g" value={item.proteinG} onChange={(event) => updateItem(item.key, { proteinG: event.target.value })} />
                <AppInput inputMode="decimal" placeholder="脂肪 g" value={item.fatG} onChange={(event) => updateItem(item.key, { fatG: event.target.value })} />
              </div>
            </div>
          </section>
        ))}
        ...
        <section className="life-surface life-section-card grid gap-2">
          <div className="flex items-center justify-between">
            ...
            <strong className="text-lg tabular-nums text-[var(--life-text)]">{preview} kcal</strong>
          </div>
