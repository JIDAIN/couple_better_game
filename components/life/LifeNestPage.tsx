    ...
    <AppPageShell title="小窝" subtitle="不常用、但属于我们生活的东西，都收在这里。">
      <section className="life-surface life-section-card overflow-hidden">
        <div className="rounded-[var(--life-radius-card)] bg-[linear-gradient(145deg,color-mix(in_srgb,var(--life-pink)_18%,white),color-mix(in_srgb,var(--life-yellow)_20%,white))] px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-base font-extrabold text-[var(--life-text)]">我们的生活角落</p>
              <p className="mt-1 text-xs leading-5 text-[var(--life-text-body)]">这里不做排名，也不把数字当成绩。</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        {tiles.map((tile) => {
          const body = (
            <>
              <div className={`grid h-12 w-12 place-items-center rounded-2xl text-2xl shadow-[var(--life-shadow-press)] ${tile.tone}`}>{tile.icon}</div>
              <div className="mt-3">
                <p className="text-sm font-extrabold text-[var(--life-text)]">{tile.title}</p>
                <p className="mt-1 text-[11px] leading-5 text-[var(--life-text-muted)]">{tile.note}</p>
              </div>
