...
function themeClass(themeKey: string) {
  return THEMES.some((theme) => theme.key === themeKey) ? `theme-${themeKey}` : "theme-cream";
}
...
export function LifeMailboxPage() {
  ...
  async function save() {
    ...
    if (form.format === "letter" && !(form.title ?? "").trim()) { setError("信纸需要一个标题"); return; }
    const ownedForm = {
      ...form,
  ...
  async function remove(letter: MailboxLetter) {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "删除信件失败"); }
  }
  ...
  return <>
    <AppPageShell title="小信箱" subtitle="列表只留标题和摘要，点开以后再慢慢读。" actions={<Link href="/nest" className="life-back-link">返回小窝</Link>}>
      <section className="life-surface life-section-card life-toolbar">
        ...
        <div className="life-mailbox-format-filter mt-3 grid grid-cols-3 gap-1 rounded-full bg-[var(--life-surface-soft)] p-1">
          {(["all", "letter", "postcard"] as FormatFilter[]).map((value) => (
            <button key={value} type="button" onClick={() => setFormatFilter(value)} className={formatFilter === value ? "is-active" : ""}>{value === "all" ? "所有" : value === "letter" ? "信纸" : "明信片"}</button>
          ))}
        </div>

        <div className="life-mailbox-filter mt-3 flex items-center justify-between gap-3">
          <div><p className="text-xs font-extrabold text-[var(--life-text)]">时间归档</p><p className="mt-0.5 text-[10px] text-[var(--life-text-muted)]">也可以按月份翻看</p></div>
          ...
          <article key={letter.id} className={`life-letter-preview ${letter.format === "postcard" ? "is-postcard" : "is-letter"} ${themeClass(letter.themeKey)}`}>
            <button type="button" className="life-letter-preview-main text-left" onClick={() => setReading(letter)}>
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="life-letter-stamp">{letter.format === "postcard" ? "明信片" : "信纸"}</span>
              </div>
              {letter.format === "letter" ? (
                <><h2 className="mt-3 line-clamp-1 text-base font-black text-[var(--life-text)]">{letter.title || "一封没有标题的信"}</h2><p className="mt-2 line-clamp-2 text-xs leading-5 text-[var(--life-text-body)]">{letter.body}</p></>
              ) : (
    ...
    {reading ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/25 p-3 sm:items-center" onMouseDown={() => setReading(null)}>
      <article className={`island-life-v2 life-letter-reader ${themeClass(reading.themeKey)}`} onMouseDown={(event) => event.stopPropagation()}>
        ...
        {reading.format === "letter" ? <h2 className="mt-5 text-xl font-black text-[var(--life-text)]">{reading.title || "一封没有标题的信"}</h2> : <p className="mt-5 text-xs font-extrabold tracking-[0.16em] text-[var(--life-text-muted)]">POSTCARD</p>}
        <p className="mt-4 whitespace-pre-wrap text-sm leading-8 text-[var(--life-text-body)]">{reading.body}</p>
        {reading.senderKey === mePartnerKey ? <button type="button" className="mt-5 text-xs font-extrabold text-[var(--life-teal-strong)]" onClick={() => openEdit(reading)}>编辑这封{reading.format === "letter" ? "信" : "明信片"}</button> : null}
      </article>
    </div> : null}
    ...
    {editing !== undefined && form ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/25 p-3 sm:items-center">
      <div className="island-life-v2 max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-[28px] bg-[var(--life-surface)] p-4 shadow-[var(--life-shadow-float)]">
        ...
        <div className="mt-4 grid gap-3">
          ...
          <div><p className="mb-1 text-xs font-bold">样式</p><div className="grid grid-cols-2 gap-2"><button onClick={() => setForm({ ...form, format: "letter" })} className={`rounded-2xl border px-3 py-3 text-sm font-bold ${form.format === "letter" ? "border-[var(--life-teal)] bg-[var(--life-surface-soft)]" : "border-[var(--life-border)]"}`}>✉️ 信纸</button><button onClick={() => setForm({ ...form, format: "postcard", title: null })} className={`rounded-2xl border px-3 py-3 text-sm font-bold ${form.format === "postcard" ? "border-[var(--life-teal)] bg-[var(--life-surface-soft)]" : "border-[var(--life-border)]"}`}>🏝️ 明信片</button></div></div>
          {form.format === "letter" ? <label className="text-xs font-bold">标题<input value={form.title ?? ""} maxLength={120} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="给这封信起个名字" className="mt-1 w-full rounded-2xl border border-[var(--life-border)] bg-white px-3 py-2.5 text-sm outline-none focus:border-[var(--life-teal)]" /></label> : null}
          ...
          <label className="text-xs font-bold">内容<textarea autoFocus value={form.body} maxLength={2000} onChange={(event) => setForm({ ...form, body: event.target.value })} rows={8} placeholder={form.format === "letter" ? "今天想告诉 Ta 什么？" : "把一句想说的话写在明信片上…"} className="mt-1 w-full resize-none rounded-[var(--life-radius-card)] border border-[var(--life-border)] bg-white px-3 py-3 text-sm leading-6 outline-none focus:border-[var(--life-teal)]" /><span className="mt-1 block text-right text-[10px] text-[var(--life-text-muted)]">{form.body.length}/2000</span></label>
