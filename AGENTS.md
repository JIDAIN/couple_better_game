...
但产品展示层应按 **`partnerKey + date`** 把同一天的 intake、deficit、weight、exercise 关联展示。关联展示不等于自动互相改值；尤其不能仅根据 meals 自动覆盖游戏 deficit。
...
上下文不明确时不得猜角色后写入。若用户在对话中明确说明角色，以用户当前明确说明为最高优先级，并同步修正文档/数据中的错误映射。
...
10. 涉及日汇总时，以 `partnerKey + date` 关联 intake 与 daily record；缺任何一侧时明确显示“未记录”，不要伪造或自动补写另一域。