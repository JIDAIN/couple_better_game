...
- **P2.5 同日关联展示**：按 `partnerKey + date` 把当天餐食总摄入/明细与当天已有的 `deficit / exercise / weight snapshot` 放到同一个日视图中。
- 缺哪一侧就显示“未记录”，不为了页面完整自动伪造另一域。
- 关联展示不等于自动计算或覆盖游戏 `deficit`。
...
但四个域在产品展示层应按 **`partnerKey + date`** 关联为“同一天”，避免餐食明细和当天游戏记录彼此割裂。
...
当前约定：**用户自己的饮食聊天映射 `cat`（猫猫），鱼鱼的饮食聊天映射 `fish`（鱼鱼）**。用户在当前对话中明确说明角色时，以当前明确说明为最高优先级。一次确认只生成一个 `chatgpt:` 幂等键；工具结果不确定时先按同 key 查询，再用同 key 重试，禁止换 key 盲目重写。
...
3. **游戏规则可回算。**
4. **云端写入有保护，不因新设备误覆盖。**
5. **UI、业务规则、服务端和数据库边界清楚。**
6. **文档描述当前真实实现，不把“未来方案”写成“已经实现”。**