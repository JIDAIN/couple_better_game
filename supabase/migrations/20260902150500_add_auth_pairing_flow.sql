-- V2-R1B: narrow invite-based pairing for Supabase Auth users.
-- Existing life-domain tables remain server-only. This migration only adds the
-- authenticated pairing boundary needed before API writes can become user-scoped.
...
create table if not exists public.couple_space_invites (
  ...
  constraint couple_space_invites_used_pair check (
    (used_at is null and used_by_user_id is null)
    or (used_at is not null and used_by_user_id is not null)
  )
);

create index if not exists couple_space_invites_space_idx
  on public.couple_space_invites(couple_space_id, expires_at desc);

alter table public.couple_space_invites enable row level security;

-- Raw invite codes are never stored. Only a SHA-256 digest is persisted.
...
begin
  ...
  where i.couple_space_id = v_space_id
    and i.invited_partner_key = p_partner_key
    and i.used_at is null;

  v_code := upper(encode(gen_random_bytes(6), 'hex'));

  insert into public.couple_space_invites(
    couple_space_id,
    created_by_user_id,
    invited_partner_key,
    code_hash,
...
begin
  ...
  if exists (select 1 from public.couple_space_members m where m.user_id = v_user_id) then
  ...
  if exists (
    select 1 from public.couple_space_members m
    where m.couple_space_id = v_invite.couple_space_id
      and m.partner_key = v_invite.invited_partner_key
  ) then
    raise exception 'PARTNER_SLOT_TAKEN';
  end if;

  insert into public.couple_space_members(
    couple_space_id,
    user_id,
    partner_key,
    member_role
  ...
  return jsonb_build_object(
    'coupleSpaceId', v_invite.couple_space_id,
    'partnerKey', v_invite.invited_partner_key
  );
end;
$$;

revoke all on public.couple_space_invites from anon, authenticated;
revoke all on function public.create_couple_space_invite(text) from public, anon;
revoke all on function public.accept_couple_space_invite(text) from public, anon;
